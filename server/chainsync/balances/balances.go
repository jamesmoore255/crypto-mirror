...
// http://localhost:8081/api/v1/wallets/tokens?addresses=0x4587FC29677eCb64bCA501a7663D3C7ebEBCb27C
func (wa WalletAddresses) FetchBalances(bClient client.BlockClient) ([]WalletBalances, error) {
	...
	metadataResp, err := erc20Tokens.Result.FetchTokensMetadata(bClient)
...
func parseBalances(erc20Tokens ERC20WalletBalances, ethBalances []client.EthBalance, metadata map[string]client.FetchMetadataResponse) ([]WalletBalances, error) {
	...
	for _, wallet := range erc20Tokens {
		...
		for _, tokenBalance := range wallet.TokenBalances {
			...
			metadata := metadata[tokenBalance.ContractAddress].Result
...
func (ws ERC20WalletBalances) FetchTokensMetadata(bClient client.BlockClient) (map[string]client.FetchMetadataResponse, error) {
